...
def get_descendant_species(tax_off, tax_tab, ctax_buff):
	...
	return np.array(sorted(leaf_traverse(tax_off, tax_tab, ctax_buff, [], append_species)), dtype=np.uint64)

# HOGs
def traverse_HOG(hog_off, hog_tab, chog_buff, preorder_fun=None, postorder_fun=None, leaf_fun=None, acc1=None, acc2=None, **kwargs):
    '''
    generic function to traverse HOGs
    '''
    if preorder_fun:
        acc1, acc2 = preorder_fun(hog_off, acc1, acc2, **kwargs)
    ...
    for chog in _children_hog(hog_off, hog_tab, chog_buff):
        ...
        if hog_tab[chog]['ChildrenHOGoff'] == -1:
            ...
            if leaf_fun:
                acc1, acc2 = leaf_fun(chog, acc1, acc2, **kwargs)
        
        else:
            traverse_HOG(chog, hog_tab, chog_buff, preorder_fun, postorder_fun, leaf_fun, acc1, acc2, **kwargs)
            
    if postorder_fun:
        acc1, acc2 = postorder_fun(hog_off, acc1, acc2, **kwargs)
...
def get_descendant_species_taxoffs(hog_off, hog_tab, chog_buff, cprot_buff, prot2speoff, speoff2taxoff):
	'''
	traverse the HOG to collect all species 
	'''
	prot_offs = get_descendant_prots(np.append(get_descendant_hogs(hog_off, hog_tab, chog_buff), np.uint64(hog_off)), hog_tab, cprot_buff)
...
def find_sisspecies(ortholog_species, inparalog_species, sispecies_cands):
    ...
    for ss_cand in sispecies_cands:
        if len(set(list(ortholog_species) + list(inparalog_species)).intersection(ss_cand)) > 0:
            sispecies = ss_cand
            break
    return sispecies
...
def compute_inparalog_coverage_new(qoff, query_ids, prot_tab, cprot_buff, hog_tab, chog_buff, hidden_taxa, sispecies_cands, verbose=0):
    ...
    for hog_off in leaf_root_hogs:
        ...
        # duplication outside hidden taxa but not knowloedgeable because of all species members are hidden
            ...
            if sis_hogs:
                ...
                sis_inhogs = list(itertools.chain(*[list(get_descendant_hogs(sh, hog_tab, chog_buff)) 
                                                    for sh in sis_hogs]))
        ...
        # HOG is knowloedgeable
            ...
            # find sister species
            sisspecies = find_sisspecies(ortholog_species, inparalog_species, sispecies_cands)
...
# def compute_inparalog_coverage(tax_off, tax2parent, hidden_taxa, tax_tab, ctax_buff, query_ids, prot_tab, hog_tab, cprot_buff, chog_buff):
# 	'''
# 	1. get sister species
# 	2. get true HOG proteins from sister species
# 	3. get descendant HOG proteins from sister species
# 	4. compute inparalog coverage
# 	'''

# 	def _compute_sisspecies_filter(prots, sisspecies, prot_tab):
# 		prots_sp = prot_tab[prots]['SpeOff']
# 		return np.array([x in sisspecies for x in prots_sp], dtype=np.uint64)

# 	# get sister species
# 	sistaxa = get_sister_taxa(tax_off, tax2parent, hidden_taxa, tax_tab, ctax_buff)
# 	sisspecies = np.array([get_descendant_species(t, tax_tab, ctax_buff) for t in sistaxa]).flatten()
	
# 	l = []
# 	for qoff in query_ids:

# 		# get proteins of true HOG from sister species
# 		ms_true_hog = prot_tab[qoff]['HOGoff']
# 		truehog_prots = _children_prot(ms_true_hog, hog_tab, cprot_buff)
# 		sisspecies_filter = _compute_sisspecies_filter(truehog_prots, sisspecies, prot_tab)
# 		truehog_prots_sispecies = truehog_prots[sisspecies_filter]

# 		# get proteins of descendant HOGs from sister species
# 		descendant_hogs = get_descendant_hogs(ms_true_hog, hog_tab, chog_buff)
# 		descendant_prots = get_descendant_prots(descendant_hogs, hog_tab, cprot_buff)
# 		sisspecies_filter = _compute_sisspecies_filter(descendant_prots, sisspecies, prot_tab)
# 		descendant_prots_sisspecies = descendant_prots[sisspecies_filter]

# 		# calculate inparalog coverage
# 		n = descendant_prots_sisspecies.size
# 		d = n + truehog_prots_sispecies.size
# 		l.append(n / d if n != 0 else 0)
