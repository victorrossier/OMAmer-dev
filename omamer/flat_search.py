...
#     def __init__(self, ki, name=None, path=None, n_top=50, matrix='BLOSUM62', 
#         gap_open=11, gap_extend=1, nthreads=None, low_mem=False):
...
#     @cached_property
#     def trans(self):
#         return get_transform(self.ki.k, self.ki.DIGITS_AA)
...
#     #######################################################################################################################################
#     ### main ###
#     def flat_search(self, seqs=None, ids=None, fasta_file=None, **kwargs):
#         '''
...
#         self._flat_search(sb, **kwargs)

#     def _flat_search(self, qbuff, **kwargs):
        
#         if self.ki.idx_type == 'lca':

#             print('look-up k-mer index')
#             fam_ranked, queryFam_count, queryFam_occur, queryHog_count, queryHog_occur, query_count, query_occur = self.lookup_lca(qbuff)
...
#         elif self.ki.idx_type == 'cs':
#             fam_results, hog_results, cs_scores = self.search_cs(qbuff, **kwargs)

#             # store results (+ self._query_id.nrows is when adding multiple fastas)
#             self._res_tab.append(list(zip(*[qbuff.ids[:].flatten(), fam_results, hog_results, cs_scores])))
...
#     ### look-up lca index ###
#     @lazy_property
#     def _lookup_lca(self):
#         def func(seqs, seqs_idx, trans, lca_idx_arr, lca_idx_off, hog2fam, n_fams, n_hogs, k, DIGITS_AA_LOOKUP):
#             '''
#             '''
#             fam_results = np.zeros((len(seqs_idx)-1, n_fams), dtype=np.uint32)
#             fam_counts = np.zeros((len(seqs_idx)-1, n_fams), dtype=np.uint16)  
...
#                 # TO DO: get locations simultaneously
#                 s_norm = DIGITS_AA_LOOKUP[s]
#                 r = np.zeros(len(s)-k+1, dtype=np.uint32) # max kmer 7
#                 for i in numba.prange(len(s)-k+1):
#                     # numba can't do np.dot with non-float
#                     for j in range(k):
#                         r[i] += trans[j] * s_norm[i+j]
#                 r1 = np.unique(r)
...
#                 for m in numba.prange(len(r1)):
#                     kmer = r1[m]
#                     x = lca_idx_off[kmer:kmer+2]
#                     hogs = lca_idx_arr[x[0]:x[1]]
#                     fams = hog2fam[hogs]
#                     hog_res[hogs] += np.uint16(1)
#                     fam_res[fams] += np.uint16(1)
#                     # kmer_occ is the nr of hogs/fams with the given kmer, used to compute its frequency
...
#     def lookup_lca(self, qbuff):
#         return self._lookup_lca(qbuff.buff, qbuff.idx, self.trans, self.idx_arr, self.idx_off, self.db._hog_tab.col('FamOff'), self.db._fam_tab.nrows, self.db._hog_tab.nrows, self.ki.k, self.ki.DIGITS_AA_LOOKUP)

#     #######################################################################################################################################
#     ### search protein index ###
#     @lazy_property
#     def _lookup_cs(self):
#         def func(seqs, seqs_idx, trans, entries, idx, k, n, top_n, DIGITS_AA_LOOKUP):
#             results = np.zeros((len(seqs_idx)-1, top_n), dtype=np.uint32)
...
#     def lookup_cs(self, qbuff):
#         return self._lookup_cs(qbuff.buff, qbuff.idx, self.trans, self.idx_arr, self.idx_off, self.ki.k, self.db._prot_tab.nrows, self.n_top, self.ki.DIGITS_AA_LOOKUP)

#     def search_cs(self, qbuff, align=True):
...
#         # k-mer search
#         (cands, counts) = self.lookup_cs(qbuff)
...
#         # hdf5 file to store CS results and query id table
#         self.file = "{}{}.h5".format(self.ki.path, self.name)
...
# class BLASTsearchValidation(BLASTsearch):
#     '''
#     to export query fasta for BLAST/DIAMOND, parse BLAST output and mimic FlatSearchValidation
#     Note, BLAST/DIAMOND have to be ran independly from the outputed fasta of ReferenceExport and BLASTsearch
#     '''
...
#             cs, score = self._get_closest(int(entry), sp_filter, prot_tab, id2off, closest)
            
#             if cs:
#                 rows.append((ids[i], prot_tab['FamOff'][cs], prot_tab['HOGoff'][cs], score))
#             else:
#                 rows.append((ids[i], -1, -1, 1000000 if prob else -1))
...
#         cs = None
#         rank = 1
#         cand, score = closest.by_dist(entry, rank=rank)
        
#         while cand:
#             f_cand = _filter_cand(cand, sp_filter, prot_tab, id2off)
#             if f_cand:
#                 cs = id2off[str(random.choice(list(f_cand))).encode('ascii')]
#                 break
#             else:
